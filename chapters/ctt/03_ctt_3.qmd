...
cat("Item paralleli:\n")
cat("Correlazione (X1, X2):", round(corr_par, 3), "\n")
cat("Affidabilità teorica  :", round(rel_par, 3), "\n")
```
...
**Risultato**: Qui la correlazione tra $X_1$ e $X_2$ non coincide con la stima di affidabilità, perché abbiamo rotto una delle condizioni di parallelismo (stessa varianza d’errore e/o zero shift). Pur avendo la stessa “pendenza” ($b=1$), lo shift e la differente varianza d’errore fanno sì che $\mathrm{corr}(X_1, X_2)\neq \mathrm{var}(T)/\mathrm{var}(X_1)$.
...
I coefficiente Alpha di Cronbach è l'indice principale fornito dalla CTT per misurare la consistenza interna. Il coefficiente Alpha è adatto per item $\tau$-equivalenti:
...
Una derivazione della formula del coefficiente alpha di Cronbach è fornita nel capitolo @sec-fa-reliability.
...
In questo contesto, **$\rho_1$** rappresenta la correlazione (o affidabilità) media di un item rispetto al costrutto che si intende misurare, mentre **$p$** è il numero di item totali. Una derivazione della formula del coefficiente di Spearman-Brown è fornita nel capitolo @sec-fa-reliability.
...
In sintesi, la **Spearman-Brown** combina il numero degli item ($p$) con l’affidabilità di un singolo item ($\rho_1$) per stimare la **consistenza interna globale** di una scala composta da item paralleli. Quando tale assunzione (parallelismo) è soddisfatta, la stima risulta particolarmente intuitiva.
...
Supponiamo di avere due subtest, ciascuno con:  
...
1. **Varianza del punteggio composito** (sola parte vera). Se $Z$ è la somma dei due subtest:  
   $$
...
library(MASS)
Sigma_true <- matrix(c(25, 15,
                       15, 25), nrow = 2)
X_true <- MASS::mvrnorm(n = n, mu = c(0,0), Sigma = Sigma_true)
...
- **Vantaggio pratico**: Un punteggio composito è spesso considerato più **robusto** e **rappresentativo** del costrutto di interesse, specialmente in ambito psicometrico, dove il singolo item o subtest può essere soggetto a un errore specifico più elevato.
...
Sigma_true <- matrix(c(var_Tx,    cov_TxTy,
                       cov_TxTy,  var_Ty),
                     nrow = 2)
```

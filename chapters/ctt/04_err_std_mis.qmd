...
**Secondo passaggio:**  
Passiamo ora alla varianza del punteggio osservato. In base alla CTT, ogni punteggio osservato $X$ può essere scomposto come somma di due componenti: punteggio vero ($T$) ed errore di misurazione ($E$):
...
- L'intervallo di confidenza corretto è quindi $\bigl[T \pm z \times \text{SEM}\bigr]$, dove $T$ è il punteggio vero e $z$ è il valore critico corrispondente al livello di confidenza desiderato.
- **Non è appropriato** calcolare l'intervallo di confidenza attorno al punteggio osservato $X$ utilizzando il SEM, poiché questo non riflette correttamente la fonte dell'incertezza nella misurazione.
...
2. Una volta stimato il punteggio vero, si può costruire l'intervallo di confidenza attorno a questa stima
...
Calcolare un intervallo di confidenza direttamente attorno al **punteggio osservato** usando il SEM è concettualmente errato. Diversi autori (ad es. *Charter, 1996; Dudek, 1979*) evidenziano che quando si vuole costruire un intervallo di confidenza partendo dal punteggio osservato, sarebbe più appropriato impiegare l'**Errore Standard della Stima (SEE)**, che riflette meglio l'incertezza nella stima del punteggio vero a partire dall'osservato. Questo aspetto sarà affrontato in modo più approfondito nel capitolo successivo.
